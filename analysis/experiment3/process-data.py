...
def get_player_data(data, inactive):
    ...
    for p in player_data:
        ...
        player_data[p].index.names = ['Index']
        columns = []
        for q in player_data:
...
def update_characteristics(player_data, p, t):
...
def process_data(data, inactive, verbose = True):
    ...
    for t in ticks:
        ...
        for p in player_data:
            update_characteristics(player_data, p, t)
    ...
    df = df.sort_values(by = ['tick','pid'])
    
    return True, df

def run(pars):
    out_dir, data_dir, data_file, inactive = pars
    if data_file[-4:] != '.csv':
        return
    ...
    success, df = process_data(data, inactive)
    if success:
...
if __name__ == '__main__':
    ...
    in_dir = '../../data/experiment3/'
    out_dir = '../../data/experiment3/processed/'
    if waits:
        out_dir += '-waits'
    if all_players:
        out_dir += '-all'
    ...
    for game_group in games:
        ...
        for data_file in os.listdir(data_dir):
            pars += [(out_dir, data_dir, data_file, inactive)]
