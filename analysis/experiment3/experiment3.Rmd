...
inactive_pids_end <- read_csv(here('data/experiment3/inactive_games.csv'), col_names = c('pid')) %>% pull(pid)
...
d.exp3 <- d.exp3.raw %>%
  ...
  # uncomment to look at results excluding *all* PIDs that went inactive (even in the last few seconds)
  #filter(!(pid %in% inactive_pids))  %>%
  # uncomment to look at results re-coding for 'final' group sizes after these dropouts
  #group_by(gameid) %>%
  #mutate(n_players = length(unique(pid)))
...
## Examine auto-correlations (Fig S6)
...
```{r}
library(lmerTest)
lmer(avg_score ~ poly(n_players, 2) * difficulty + (1 | gameid) + (1 | noise:difficulty),
...
```{r}
d %>%
  glmer(exploit ~ bg_val * difficulty + (1 | pid) + (1 | noise),
        family = 'binomial',
        contrasts = list(difficulty=contr.sum(2)),
        data = .) %>%
  summary()
...
```{r}
d %>%
  glmer(copy ~ bg_val * difficulty + (1 | pid) + (1 | noise),
        family = 'binomial',
        contrasts = list(difficulty=contr.sum(2)),
        data = .) %>%
  summary()
...
```{r}
d %>%
  glmer(explore ~ bg_val * difficulty + (1 | pid) + (1 | noise),
        family = 'binomial',
        contrasts = list(difficulty=contr.sum(2)),
        data = .) %>%
  summary()
