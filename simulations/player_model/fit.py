...
class Fit():
    ...
    def __init__(self, game_data = None, background_dir = None, player = None, debug = False, out_dir = None, test = False, reps = 30):
        ...
        if game_data is not None:
            ...
            self.world = World(noise_location = background_dir, n_players = self.n_players)
            
            self.window = 160
            self.start = 0#self.world.game_length / 3 - 1
            self.stop_training = self.world.game_length
        ...
        self.reps = reps
        self.pars = [self.prior() for i in range(self.reps)]
    ...
    def fit_model(self, model_type):
        ...
        self.models = np.array([model_type(self.prior()) for i in range(self.reps)])
        ...
        for t in range(self.world.game_length):
            ...
            if t >= self.start:
                ...
                if t % 100 == 0:
                    print np.mean([self.models[i].goal_dist for i in range(len(self.models))])
                    #sys.stdout.write('.')
        ...
        best_par = np.mean([self.models[i].goal_dist for i in range(len(self.models))])
    ...
    def make_predictions(self, t):
        ...
        errs = []
        for i in range(len(self.models)):
            self.models[i].goal_dist += np.random.normal()
            model_traj, real_traj = self.predict_forwards(i, t, return_full = True)
            errs += [np.linalg.norm(model_traj - real_traj)]
        
        weights = -np.array(errs)
        norm = logsumexp(weights)
        weights -= norm
        weights = np.exp(weights)

        inds = np.random.choice(len(weights), size = len(self.models), p = weights)
        
        self.models = copy.deepcopy(self.models[inds])
