...
for n in sorted(list(set(data['n_players']))):
    ...
    ax.plot(sub['mean_lat'], sub['score'], marker=shapes[i], linestyle='', ms = 10, label = n)
    i += 1
plt.legend(bbox_to_anchor=(1.05, 1), loc=2, borderaxespad=0.)

plt.xlabel('Mean Latency', fontsize=24)
...
fig, ax = plt.subplots()
ax.margins(0.05)
ax.plot(data['n_players'], data['mean_lat'], marker='o', linestyle='', ms = 10)

plt.xlabel('Number of Players', fontsize=24)
plt.ylabel('Log Latency', fontsize=24)
